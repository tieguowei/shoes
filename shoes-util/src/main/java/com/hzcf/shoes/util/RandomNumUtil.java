 ...
 * ============================================================
 *
 * FileName: RandomNumUtil.java 
 *
 * Created: [2015年5月8日 上午11:53:27] by jie 
 *
 * $Id$
 * 
 * $Revision$
...
public class RandomNumUtil {
	...
	static ArrayList<Character> lower = null;
	static ArrayList<Character> upper = null;
	 ...
	 * 
	 */
	public static void setRanges(ArrayList<Character> low, ArrayList<Character> up) {
		lower = low;
		upper = up;
	}
	 ...
	 * 
	 */
	public static void setHmap(HashMap<String, Object> map) {
		hmap = map;
	}
	...
	public static void setSingle(char[] chars, int value) {
		char[] tmp = chars;
		single = tmp; // set the array of chars
	 ...
	 * @throws Exception
	 */
	public static String getRandomNum(String size){
		try {
			RandomNumUtil.setCharset("0-9");//指定字符内容为"0"-"z"，当然也可以是别的，比如"a-z"或"A-Z" 
			RandomNumUtil.setLength(size);//指定字符长度 
			RandomNumUtil.generateRandomObject();//生成随机数 
			return RandomNumUtil.getRandom();//得到随机数
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
	...
	public static String getRandomNumOfHxPwd()throws Exception {
		RandomNumUtil.setCharset("0-z");//指定字符内容为"0"-"z"，当然也可以是别的，比如"a-z"或"A-Z" 
		RandomNumUtil.setLength("20");//指定字符长度 
		RandomNumUtil.generateRandomObject();//生成随机数 
		String str = RandomNumUtil.getRandom();//得到随机数 
		return str;
	}
}
