package com.hzcf.shoes.util;

import java.util.ArrayList;
import java.util.Comparator;
import java.util.List;
import java.util.Set;
import java.util.TreeSet;

/**
 * 
 *
 * Description: 对数组的操作的工具类
 *
 * @author yaodawei
 * @version 1.0
 * <pre>
 * Modification History: 
 * Date         Author      Version     Description 
 * ------------------------------------------------------------------ 
 * 2014-1-7    yaodawei       1.0        1.0 Version 
 * </pre>
 */
public class ArrayUtil {
	
	/**
	 * 在一个boolean[]数组后面追加一个元素
	 * @param array 原始的数组
	 * @param value 需要追加在原始数组后的元素
	 * @return boolean[]
	 */
	public static boolean[] append(boolean[] array, boolean value) {
		boolean[] newArray = new boolean[array.length + 1];
		System.arraycopy(array, 0, newArray, 0, array.length);
		newArray[newArray.length - 1] = value;
		return newArray;
	}

	/**
	 * 在一个byte[]数组后面追加一个元素
	 * @param array 原始的数组
	 * @param value 需要追加在原始数组后的元素
	 * @return byte[]
	 */
	public static byte[] append(byte[] array, byte value) {
		byte[] newArray = new byte[array.length + 1];
		System.arraycopy(array, 0, newArray, 0, array.length);
		newArray[newArray.length - 1] = value;
		return newArray;
	}

	/**
	 * 在一个double[]数组后面追加一个元素
	 * @param array 原始的数组
	 * @param value 需要追加在原始数组后的元素
	 * @return double[]
	 */
	public static double[] append(double[] array, double value) {
		double[] newArray = new double[array.length + 1];
		System.arraycopy(array, 0, newArray, 0, array.length);
		newArray[newArray.length - 1] = value;
		return newArray;
	}

	/**
	 * 在一个float[]数组后面追加一个元素
	 * @param array 原始的数组
	 * @param value 需要追加在原始数组后的元素
	 * @return float[]
	 */
	public static float[] append(float[] array, float value) {
		float[] newArray = new float[array.length + 1];
		System.arraycopy(array, 0, newArray, 0, array.length);
		newArray[newArray.length - 1] = value;
		return newArray;
	}

	/**
	 * 在一个int[]数组后面追加一个元素
	 * @param array 原始的数组
	 * @param value 需要追加在原始数组后的元素
	 * @return int[]
	 */
	public static int[] append(int[] array, int value) {
		int[] newArray = new int[array.length + 1];
		System.arraycopy(array, 0, newArray, 0, array.length);
		newArray[newArray.length - 1] = value;
		return newArray;
	}

	/**
	 * 在一个long[]数组后面追加一个元素
	 * @param array 原始的数组
	 * @param value 需要追加在原始数组后的元素
	 * @return long[]
	 */
	public static long[] append(long[] array, long value) {
		long[] newArray = new long[array.length + 1];
		System.arraycopy(array, 0, newArray, 0, array.length);
		newArray[newArray.length - 1] = value;
		return newArray;
	}

	/**
	 * 在一个short[]数组后面追加一个元素
	 * @param array 原始的数组
	 * @param value 需要追加在原始数组后的元素
	 * @return short[]
	 */
	public static short[] append(short[] array, short value) {
		short[] newArray = new short[array.length + 1];
		System.arraycopy(array, 0, newArray, 0, array.length);
		newArray[newArray.length - 1] = value;
		return newArray;
	}

	/**
	 * 在一个Boolean[]数组后面追加一个元素
	 * @param array 原始的数组
	 * @param value 需要追加在原始数组后的元素
	 * @return Boolean[]
	 */
	public static Boolean[] append(Boolean[] array, Boolean value) {
		Boolean[] newArray = new Boolean[array.length + 1];
		System.arraycopy(array, 0, newArray, 0, array.length);
		newArray[newArray.length - 1] = value;
		return newArray;
	}

	/**
	 * 在一个Double[]数组后面追加一个元素
	 * @param array 原始的数组
	 * @param value 需要追加在原始数组后的元素
	 * @return Double[]
	 */
	public static Double[] append(Double[] array, Double value) {
		Double[] newArray = new Double[array.length + 1];
		System.arraycopy(array, 0, newArray, 0, array.length);
		newArray[newArray.length - 1] = value;
		return newArray;
	}

	/**
	 * 在一个Float[]数组后面追加一个元素
	 * @param array 原始的数组
	 * @param value 需要追加在原始数组后的元素
	 * @return Float[]
	 */
	public static Float[] append(Float[] array, Float value) {
		Float[] newArray = new Float[array.length + 1];
		System.arraycopy(array, 0, newArray, 0, array.length);
		newArray[newArray.length - 1] = value;
		return newArray;
	}

	/**
	 * 在一个Integer[]数组后面追加一个元素
	 * @param array 原始的数组
	 * @param value 需要追加在原始数组后的元素
	 * @return Integer[]
	 */
	public static Integer[] append(Integer[] array, Integer value) {
		Integer[] newArray = new Integer[array.length + 1];
		System.arraycopy(array, 0, newArray, 0, array.length);
		newArray[newArray.length - 1] = value;
		return newArray;
	}

	/**
	 * 在一个Long[]数组后面追加一个元素
	 * @param array 原始的数组
	 * @param value 需要追加在原始数组后的元素
	 * @return Long[]
	 */
	public static Long[] append(Long[] array, Long value) {
		Long[] newArray = new Long[array.length + 1];
		System.arraycopy(array, 0, newArray, 0, array.length);
		newArray[newArray.length - 1] = value;
		return newArray;
	}

	/**
	 * 在一个Object[]数组后面追加一个元素
	 * @param array 原始的数组
	 * @param value 需要追加在原始数组后的元素
	 * @return Object[]
	 */
	public static Object[] append(Object[] array, Object value) {
		Object[] newArray = new Object[array.length + 1];
		System.arraycopy(array, 0, newArray, 0, array.length);
		newArray[newArray.length - 1] = value;
		return newArray;
	}

	/**
	 * 在一个Object[][]数组后面追加一个元素
	 * @param array 原始的数组（二维数组，它的元素为Object[]）
	 * @param value  需要追加在原始数组后的元素
	 * @return Object[][]
	 */
	public static Object[][] append(Object[][] array, Object[] value) {
		Object[][] newArray = new Object[array.length + 1][];
		System.arraycopy(array, 0, newArray, 0, array.length);
		newArray[newArray.length - 1] = value;
		return newArray;
	}

	/**
	 * 在一个Short[]数组后面追加一个元素
	 * @param array 原始的数组
	 * @param value 需要追加在原始数组后的元素
	 * @return Short[]
	 */
	public static Short[] append(Short[] array, Short value) {
		Short[] newArray = new Short[array.length + 1];
		System.arraycopy(array, 0, newArray, 0, array.length);
		newArray[newArray.length - 1] = value;
		return newArray;
	}

	/**
	 * 在一个String[]数组后面追加一个元素
	 *
	 * @param array
	 *            原始的数组
	 * @param value
	 *            需要追加在原始数组后的元素
	 * @return Short[]
	 */
	public static String[] append(String[] array, String value) {
		String[] newArray = new String[array.length + 1];

		System.arraycopy(array, 0, newArray, 0, array.length);

		newArray[newArray.length - 1] = value;

		return newArray;
	}

	/**
	 * 在一个String[][]数组后面追加一个元素
	 *
	 * @param array
	 *            原始的数组（二维数组，它的元素为String[]）
	 * @param value
	 *            需要追加在原始数组后的元素
	 * @return String[][]
	 */
	public static String[][] append(String[][] array, String[] value) {
		String[][] newArray = new String[array.length + 1][];

		System.arraycopy(array, 0, newArray, 0, array.length);

		newArray[newArray.length - 1] = value;

		return newArray;
	}

	/**
	 * 在一个String[]数组后面追加一个String[]数组
	 *
	 * @param array1
	 *            原始的数组
	 * @param array2
	 *            需要追加在原始数组后的数组
	 * @return boolean[]
	 */
	public static boolean[] append(boolean[] array1, boolean[] array2) {
		boolean[] newArray = new boolean[array1.length + array2.length];

		System.arraycopy(array1, 0, newArray, 0, array1.length);
		System.arraycopy(array2, 0, newArray, array1.length, array2.length);

		return newArray;
	}

	/**
	 * 在一个byte[]数组后面追加一个byte[]数组
	 *
	 * @param array1
	 *            原始的数组
	 * @param array2
	 *            需要追加在原始数组后的数组
	 * @return byte[]
	 */
	public static byte[] append(byte[] array1, byte[] array2) {
		byte[] newArray = new byte[array1.length + array2.length];

		System.arraycopy(array1, 0, newArray, 0, array1.length);
		System.arraycopy(array2, 0, newArray, array1.length, array2.length);

		return newArray;
	}

	/**
	 * 在一个double[]数组后面追加一个double[]数组
	 *
	 * @param array1
	 *            原始的数组
	 * @param array2
	 *            需要追加在原始数组后的数组
	 * @return double[]
	 */
	public static double[] append(double[] array1, double[] array2) {
		double[] newArray = new double[array1.length + array2.length];

		System.arraycopy(array1, 0, newArray, 0, array1.length);
		System.arraycopy(array2, 0, newArray, array1.length, array2.length);

		return newArray;
	}

	/**
	 * 在一个float[]数组后面追加一个float[]数组
	 *
	 * @param array1
	 *            原始的数组
	 * @param array2
	 *            需要追加在原始数组后的数组
	 * @return float[]
	 */
	public static float[] append(float[] array1, float[] array2) {
		float[] newArray = new float[array1.length + array2.length];

		System.arraycopy(array1, 0, newArray, 0, array1.length);
		System.arraycopy(array2, 0, newArray, array1.length, array2.length);

		return newArray;
	}

	/**
	 * 在一个int[]数组后面追加一个int[]数组
	 *
	 * @param array1
	 *            原始的数组
	 * @param array2
	 *            需要追加在原始数组后的数组
	 * @return int[]
	 */
	public static int[] append(int[] array1, int[] array2) {
		int[] newArray = new int[array1.length + array2.length];

		System.arraycopy(array1, 0, newArray, 0, array1.length);
		System.arraycopy(array2, 0, newArray, array1.length, array2.length);

		return newArray;
	}

	/**
	 * 在一个long[]数组后面追加一个long[]数组
	 *
	 * @param array1
	 *            原始的数组
	 * @param array2
	 *            需要追加在原始数组后的数组
	 * @return long[]
	 */
	public static long[] append(long[] array1, long[] array2) {
		long[] newArray = new long[array1.length + array2.length];

		System.arraycopy(array1, 0, newArray, 0, array1.length);
		System.arraycopy(array2, 0, newArray, array1.length, array2.length);

		return newArray;
	}

	/**
	 * 在一个short[]数组后面追加一个short[]数组
	 *
	 * @param array1
	 *            原始的数组
	 * @param array2
	 *            需要追加在原始数组后的数组
	 * @return short[]
	 */
	public static short[] append(short[] array1, short[] array2) {
		short[] newArray = new short[array1.length + array2.length];

		System.arraycopy(array1, 0, newArray, 0, array1.length);
		System.arraycopy(array2, 0, newArray, array1.length, array2.length);

		return newArray;
	}

	/**
	 * 在一个Boolean[]数组后面追加一个Boolean[]数组
	 *
	 * @param array1
	 *            原始的数组
	 * @param array2
	 *            需要追加在原始数组后的数组
	 * @return Boolean[]
	 */
	public static Boolean[] append(Boolean[] array1, Boolean[] array2) {
		Boolean[] newArray = new Boolean[array1.length + array2.length];

		System.arraycopy(array1, 0, newArray, 0, array1.length);
		System.arraycopy(array2, 0, newArray, array1.length, array2.length);

		return newArray;
	}

	/**
	 * 在一个Double[]数组后面追加一个Double[]数组
	 *
	 * @param array1
	 *            原始的数组
	 * @param array2
	 *            需要追加在原始数组后的数组
	 * @return Double[]
	 */
	public static Double[] append(Double[] array1, Double[] array2) {
		Double[] newArray = new Double[array1.length + array2.length];

		System.arraycopy(array1, 0, newArray, 0, array1.length);
		System.arraycopy(array2, 0, newArray, array1.length, array2.length);

		return newArray;
	}

	/**
	 * 在一个Float[]数组后面追加一个Float[]数组
	 *
	 * @param array1
	 *            原始的数组
	 * @param array2
	 *            需要追加在原始数组后的数组
	 * @return Float[]
	 */
	public static Float[] append(Float[] array1, Float[] array2) {
		Float[] newArray = new Float[array1.length + array2.length];

		System.arraycopy(array1, 0, newArray, 0, array1.length);
		System.arraycopy(array2, 0, newArray, array1.length, array2.length);

		return newArray;
	}

	/**
	 * 在一个Integer[]数组后面追加一个Integer[]数组
	 *
	 * @param array1
	 *            原始的数组
	 * @param array2
	 *            需要追加在原始数组后的数组
	 * @return Integer[]
	 */
	public static Integer[] append(Integer[] array1, Integer[] array2) {
		Integer[] newArray = new Integer[array1.length + array2.length];

		System.arraycopy(array1, 0, newArray, 0, array1.length);
		System.arraycopy(array2, 0, newArray, array1.length, array2.length);

		return newArray;
	}

	/**
	 * 在一个Long[]数组后面追加一个Long[]数组
	 *
	 * @param array1
	 *            原始的数组
	 * @param array2
	 *            需要追加在原始数组后的数组
	 * @return Long[]
	 */
	public static Long[] append(Long[] array1, Long[] array2) {
		Long[] newArray = new Long[array1.length + array2.length];

		System.arraycopy(array1, 0, newArray, 0, array1.length);
		System.arraycopy(array2, 0, newArray, array1.length, array2.length);

		return newArray;
	}

	/**
	 * 在一个Object[]数组后面追加一个Object[]数组
	 *
	 * @param array1
	 *            原始的数组
	 * @param array2 
	 *            需要追加在原始数组后的数组
	 * @return Object[]
	 */
	public static Object[] append(Object[] array1, Object[] array2) {
		Object[] newArray = new Object[array1.length + array2.length];

		System.arraycopy(array1, 0, newArray, 0, array1.length);
		System.arraycopy(array2, 0, newArray, array1.length, array2.length);

		return newArray;
	}

	/**
	 * 在一个Object[][]数组后面追加一个Object[][]数组
	 *
	 * @param array1
	 *            原始的数组
	 * @param array2
	 *            需要追加在原始数组后的数组
	 * @return Object[][]
	 */
	public static Object[][] append(Object[][] array1, Object[][] array2) {
		Object[][] newArray = new Object[array1.length + array2.length][];

		System.arraycopy(array1, 0, newArray, 0, array1.length);
		System.arraycopy(array2, 0, newArray, array1.length, array2.length);

		return newArray;
	}

	/**
	 * 在一个Short[]数组后面追加一个Short[]数组
	 *
	 * @param array1
	 *            原始的数组
	 * @param array2
	 *            需要追加在原始数组后的数组
	 * @return Short[
	 */
	public static Short[] append(Short[] array1, Short[] array2) {
		Short[] newArray = new Short[array1.length + array2.length];

		System.arraycopy(array1, 0, newArray, 0, array1.length);
		System.arraycopy(array2, 0, newArray, array1.length, array2.length);

		return newArray;
	}

	/**
	 * 在一个String[]数组后面追加一个String[]数组
	 *
	 * @param array1
	 *            原始的数组
	 * @param array2
	 *            需要追加在原始数组后的数组
	 * @return String[]
	 */
	public static String[] append(String[] array1, String[] array2) {
		String[] newArray = new String[array1.length + array2.length];

		System.arraycopy(array1, 0, newArray, 0, array1.length);
		System.arraycopy(array2, 0, newArray, array1.length, array2.length);

		return newArray;
	}

	/**
	 * 在一个String[][]数组后面追加一个String[][]数组
	 *
	 * @param array1
	 *            原始的数组
	 * @param array2
	 *            需要追加在原始数组后的数组
	 * @return String[][]
	 */
	public static String[][] append(String[][] array1, String[][] array2) {
		String[][] newArray = new String[array1.length + array2.length][];

		System.arraycopy(array1, 0, newArray, 0, array1.length);
		System.arraycopy(array2, 0, newArray, array1.length, array2.length);

		return newArray;
	}
	/**
	 * 
	 * Description: 
	 *
	 * @param array1 
	 * @param array2 
	 * @param combinedArray 
	 * @Author leichunlai
	 * @Create Date: 2013-2-26 下午05:06:52
	 */
	public static void combine(Object[] array1, Object[] array2,
			Object[] combinedArray) {

		System.arraycopy(array1, 0, combinedArray, 0, array1.length);

		System
				.arraycopy(array2, 0, combinedArray, array1.length,
						array2.length);
	}

	/**
	 * 判断给定的数组array中是否包含给定的元素value
	 *
	 * @param array
	 *            给定的数组
	 * @param value
	 *            给定的元素
	 * @return boolean
	 */
	public static boolean contains(boolean[] array, boolean value) {
		if ((array == null) || (array.length == 0)) {
			return false;
		} else {
			for (int i = 0; i < array.length; i++) {
				if (value == array[i]) {
					return true;
				}
			}

			return false;
		}
	}

	/**
	 * 判断给定的数组array中是否包含给定的元素value
	 *
	 * @param array
	 *            给定的数组
	 * @param value
	 *            给定的元素
	 * @return boolean
	 */
	public static boolean contains(byte[] array, byte value) {
		if ((array == null) || (array.length == 0)) {
			return false;
		} else {
			for (int i = 0; i < array.length; i++) {
				if (value == array[i]) {
					return true;
				}
			}

			return false;
		}
	}

	/**
	 * 判断给定的数组array中是否包含给定的元素value
	 *
	 * @param array
	 *            给定的数组
	 * @param value
	 *            给定的元素
	 * @return boolean
	 */
	public static boolean contains(char[] array, char value) {
		if ((array == null) || (array.length == 0)) {
			return false;
		} else {
			for (int i = 0; i < array.length; i++) {
				if (value == array[i]) {
					return true;
				}
			}

			return false;
		}
	}

	/**
	 * 判断给定的数组array中是否包含给定的元素value
	 *
	 * @param array
	 *            给定的数组
	 * @param value
	 *            给定的元素
	 * @return boolean
	 */
	public static boolean contains(double[] array, double value) {
		if ((array == null) || (array.length == 0)) {
			return false;
		} else {
			for (int i = 0; i < array.length; i++) {
				if (value == array[i]) {
					return true;
				}
			}

			return false;
		}
	}

	/**
	 * 判断给定的数组array中是否包含给定的元素value
	 *
	 * @param array
	 *            给定的数组
	 * @param value
	 *            给定的元素
	 * @return boolean
	 */
	public static boolean contains(int[] array, int value) {
		if ((array == null) || (array.length == 0)) {
			return false;
		} else {
			for (int i = 0; i < array.length; i++) {
				if (value == array[i]) {
					return true;
				}
			}

			return false;
		}
	}

	/**
	 * 判断给定的数组array中是否包含给定的元素value
	 *
	 * @param array
	 *            给定的数组
	 * @param value
	 *            给定的元素
	 * @return boolean
	 */
	public static boolean contains(long[] array, long value) {
		if ((array == null) || (array.length == 0)) {
			return false;
		} else {
			for (int i = 0; i < array.length; i++) {
				if (value == array[i]) {
					return true;
				}
			}

			return false;
		}
	}

	/**
	 * 判断给定的数组array中是否包含给定的元素value
	 *
	 * @param array
	 *            给定的数组
	 * @param value
	 *            给定的元素
	 * @return boolean
	 */
	public static boolean contains(short[] array, short value) {
		if ((array == null) || (array.length == 0)) {
			return false;
		} else {
			for (int i = 0; i < array.length; i++) {
				if (value == array[i]) {
					return true;
				}
			}

			return false;
		}
	}

	/**
	 * 判断给定的数组array中是否包含给定的元素value
	 *
	 * @param array
	 *            给定的数组
	 * @param value
	 *            给定的元素
	 * @return boolean
	 */
	public static boolean contains(Object[] array, Object value) {
		if ((array == null) || (array.length == 0) || (value == null)) {
			return false;
		} else {
			for (int i = 0; i < array.length; i++) {
				if (value.equals(array[i])) {
					return true;
				}
			}

			return false;
		}
	}

	/**
	 * 消除给定的数组array里的重复元素
	 *
	 * @param array 
	 * @return String[]
	 */
	public static String[] distinct(String[] array) {
		return distinct(array, null);
	}
	/**
	 * 
	 * Description: 
	 *
	 * @param array 
	 * @param comparator 
	 * @return String[]
	 * @Author leichunlai
	 * @Create Date: 2013-2-26 下午05:08:11
	 */
	public static String[] distinct(String[] array,
			Comparator<String> comparator) {

		if ((array == null) || (array.length == 0)) {
			return array;
		}

		Set<String> set = null;

		if (comparator == null) {
			set = new TreeSet<String>();
		} else {
			set = new TreeSet<String>(comparator);
		}

		for (int i = 0; i < array.length; i++) {
			String s = array[i];

			if (!set.contains(s)) {
				set.add(s);
			}
		}

		return set.toArray(new String[set.size()]);
	}

	/**
	 * 获得给定的数组array的长度
	 *
	 * @param array 
	 * @return int
	 */
	public static int getLength(Object[] array) {
		if (array == null) {
			return 0;
		} else {
			return array.length;
		}
	}

	/**
	 * 获得给定数组array指定下标的元素
	 *
	 * @param array
	 *            给定的数组
	 * @param pos
	 *            数组的下标
	 * @return Object
	 */
	public static Object getValue(Object[] array, int pos) {
		if ((array == null) || (array.length <= pos)) {
			return null;
		} else {
			return array[pos];
		}
	}

	/**
	 * 删除数组array中，元素的值与value相等的元素
	 *
	 * @param array
	 *            数组
	 * @param value 
	 * @return boolean[]
	 */
	public static boolean[] remove(boolean[] array, boolean value) {
		List<Boolean> list = new ArrayList<Boolean>();

		for (int i = 0; i < array.length; i++) {
			if (value != array[i]) {
				list.add(array[i]);
			}
		}

		return toArray(list.toArray(new Boolean[list.size()]));
	}

	/**
	 * 删除数组array中，元素的值与value相等的元素
	 *
	 * @param array
	 *            数组
	 * @param value 
	 * @return byte[]
	 */
	public static byte[] remove(byte[] array, byte value) {
		List<Byte> list = new ArrayList<Byte>();

		for (int i = 0; i < array.length; i++) {
			if (value != array[i]) {
				list.add(new Byte(array[i]));
			}
		}

		return toArray(list.toArray(new Byte[list.size()]));
	}

	/**
	 * 删除数组array中，元素的值与value相等的元素
	 *
	 * @param array
	 *            数组
	 * @param value 
	 * @return char[]
	 */
	public static char[] remove(char[] array, char value) {
		List<Character> list = new ArrayList<Character>();

		for (int i = 0; i < array.length; i++) {
			if (value != array[i]) {
				list.add(new Character(array[i]));
			}
		}

		return toArray(list.toArray(new Character[list.size()]));
	}

	/**
	 * 删除数组array中，元素的值与value相等的元素
	 *
	 * @param array
	 *            数组
	 * @param value 
	 * @return double[]
	 */
	public static double[] remove(double[] array, double value) {
		List<Double> list = new ArrayList<Double>();

		for (int i = 0; i < array.length; i++) {
			if (value != array[i]) {
				list.add(new Double(array[i]));
			}
		}

		return toArray(list.toArray(new Double[list.size()]));
	}

	/**
	 * 删除数组array中，元素的值与value相等的元素
	 *
	 * @param array
	 *            数组
	 * @param value 
	 * @return int[]
	 */
	public static int[] remove(int[] array, int value) {
		List<Integer> list = new ArrayList<Integer>();

		for (int i = 0; i < array.length; i++) {
			if (value != array[i]) {
				list.add(new Integer(array[i]));
			}
		}

		return toArray(list.toArray(new Integer[list.size()]));
	}

	/**
	 * 删除数组array中，元素的值与value相等的元素
	 *
	 * @param array
	 *            数组
	 * @param value 
	 * @return long[]
	 */
	public static long[] remove(long[] array, long value) {
		List<Long> list = new ArrayList<Long>();

		for (int i = 0; i < array.length; i++) {
			if (value != array[i]) {
				list.add(new Long(array[i]));
			}
		}

		return toArray(list.toArray(new Long[list.size()]));
	}

	/**
	 * 删除数组array中，元素的值与value相等的元素
	 *
	 * @param array
	 *            数组
	 * @param value 
	 * @return short[]
	 */
	public static short[] remove(short[] array, short value) {
		List<Short> list = new ArrayList<Short>();

		for (int i = 0; i < array.length; i++) {
			if (value != array[i]) {
				list.add(new Short(array[i]));
			}
		}

		return toArray(list.toArray(new Short[list.size()]));
	}

	/**
	 * 删除数组array中，元素的值与value相等的元素
	 *
	 * @param array
	 *            数组
	 * @param value 
	 * @return String[]
	 */
	public static String[] remove(String[] array, String value) {
		List<String> list = new ArrayList<String>();

		for (String s : array) {
			if (!s.equals(value)) {
				list.add(s);
			}
		}

		return list.toArray(new String[list.size()]);
	}

	/**
	 * 给定的String[]，如果数组的原属以prefix的值开始，则删除此元素。 例如 prefix = "abc",
	 * 则String[]中，所有以abc开始的元素，都从数组中删除.
	 *
	 * @param array
	 *            给定的数组
	 * @param prefix  
	 * @return String[]
	 */
	public static String[] removeByPrefix(String[] array, String prefix) {
		List<String> list = new ArrayList<String>();

		for (String s : array) {
			if (!s.startsWith(prefix)) {
				list.add(s);
			}
		}

		return list.toArray(new String[list.size()]);
	}

	/**
	 * 将boolean[] 数组转化成Boolean[]数组
	 *
	 * @param array 
	 * @return Boolean[]
	 */
	public static Boolean[] toArray(boolean[] array) {
		Boolean[] newArray = new Boolean[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = Boolean.valueOf(array[i]);
		}

		return newArray;
	}

	/**
	 * 将byte[] 数组转化成Byte[]数组
	 *
	 * @param array 
	 * @return Byte[]
	 */
	public static Byte[] toArray(byte[] array) {
		Byte[] newArray = new Byte[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = Byte.valueOf(array[i]);
		}

		return newArray;
	}

	/**
	 * 将char[] 数组转化成Character[]数组
	 *
	 * @param array 
	 * @return Character[]
	 */
	public static Character[] toArray(char[] array) {
		Character[] newArray = new Character[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = Character.valueOf(array[i]);
		}

		return newArray;
	}

	/**
	 * 将double[] 数组转化成Double[]数组
	 *
	 * @param array 
	 * @return Double[]
	 */
	public static Double[] toArray(double[] array) {
		Double[] newArray = new Double[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = new Double(array[i]);
		}

		return newArray;
	}

	/**
	 * 将float[] 数组转化成Float[]数组
	 *
	 * @param array 
	 * @return Float[]
	 */
	public static Float[] toArray(float[] array) {
		Float[] newArray = new Float[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = new Float(array[i]);
		}

		return newArray;
	}

	/**
	 * 将int[] 数组转化成Integer[]数组
	 *
	 * @param array 
	 * @return Integer[]
	 */
	public static Integer[] toArray(int[] array) {
		Integer[] newArray = new Integer[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = new Integer(array[i]);
		}

		return newArray;
	}

	/**
	 * 将long[] 数组转化成Long[]数组
	 *
	 * @param array 
	 * @return Long[]
	 */
	public static Long[] toArray(long[] array) {
		Long[] newArray = new Long[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = new Long(array[i]);
		}

		return newArray;
	}

	/**
	 * 将short[] 数组转化成Short[]数组
	 *
	 * @param array 
	 * @return Short[]
	 */
	public static Short[] toArray(short[] array) {
		Short[] newArray = new Short[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = new Short(array[i]);
		}

		return newArray;
	}

	/**
	 * 将Boolean[] 数组转化成boolean[]数组
	 *
	 * @param array 
	 * @return boolean[]
	 */
	public static boolean[] toArray(Boolean[] array) {
		boolean[] newArray = new boolean[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = array[i].booleanValue();
		}

		return newArray;
	}

	/**
	 * 将Byte[] 数组转化成byte[]数组
	 *
	 * @param array 
	 * @return byte[]
	 */
	public static byte[] toArray(Byte[] array) {
		byte[] newArray = new byte[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = array[i].byteValue();
		}

		return newArray;
	}

	/**
	 * 将Character[] 数组转化成byte[]数组
	 *
	 * @param array 
	 * @return char[]
	 */
	public static char[] toArray(Character[] array) {
		char[] newArray = new char[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = array[i].charValue();
		}

		return newArray;
	}

	/**
	 * 将Double[] 数组转化成double[]数组
	 *
	 * @param array 
	 * @return double[]
	 */
	public static double[] toArray(Double[] array) {
		double[] newArray = new double[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = array[i].doubleValue();
		}

		return newArray;
	}

	/**
	 * 将Float[] 数组转化成float[]数组
	 *
	 * @param array 
	 * @return float[]
	 */
	public static float[] toArray(Float[] array) {
		float[] newArray = new float[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = array[i].floatValue();
		}

		return newArray;
	}

	/**
	 * 将Integer[] 数组转化成int[]数组
	 *
	 * @param array 
	 * @return int[]
	 */
	public static int[] toArray(Integer[] array) {
		int[] newArray = new int[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = array[i].intValue();
		}

		return newArray;
	}

	/**
	 * 将Long[] 数组转化成long[]数组
	 *
	 * @param array 
	 * @return long[]
	 */
	public static long[] toArray(Long[] array) {
		long[] newArray = new long[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = array[i].longValue();
		}

		return newArray;
	}

	/**
	 * 将Short[] 数组转化成short[]数组
	 *
	 * @param array 
	 * @return short[]
	 */
	public static short[] toArray(Short[] array) {
		short[] newArray = new short[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = array[i].shortValue();
		}

		return newArray;
	}

	/**
	 * 将boolean[] 数组转化成String[]数组
	 *
	 * @param array 
	 * @return String[]
	 */
	public static String[] toStringArray(boolean[] array) {
		String[] newArray = new String[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = String.valueOf(array[i]);
		}

		return newArray;
	}

	/**
	 * 将byte[] 数组转化成String[]数组
	 *
	 * @param array 
	 * @return String[]
	 */
	public static String[] toStringArray(byte[] array) {
		String[] newArray = new String[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = String.valueOf(array[i]);
		}

		return newArray;
	}

	/**
	 * 将char[] 数组转化成String[]数组
	 *
	 * @param array 
	 * @return String[]
	 */
	public static String[] toStringArray(char[] array) {
		String[] newArray = new String[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = String.valueOf(array[i]);
		}

		return newArray;
	}

	/**
	 * 将double[] 数组转化成String[]数组
	 *
	 * @param array 
	 * @return String[]
	 */
	public static String[] toStringArray(double[] array) {
		String[] newArray = new String[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = String.valueOf(array[i]);
		}

		return newArray;
	}

	/**
	 * 将float[] 数组转化成String[]数组
	 *
	 * @param array 
	 * @return String[]
	 */
	public static String[] toStringArray(float[] array) {
		String[] newArray = new String[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = String.valueOf(array[i]);
		}

		return newArray;
	}

	/**
	 * 将int[] 数组转化成String[]数组
	 *
	 * @param array 
	 * @return String[]
	 */
	public static String[] toStringArray(int[] array) {
		String[] newArray = new String[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = String.valueOf(array[i]);
		}

		return newArray;
	}

	/**
	 * 将long[] 数组转化成String[]数组
	 *
	 * @param array 
	 * @return String[]
	 */
	public static String[] toStringArray(long[] array) {
		String[] newArray = new String[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = String.valueOf(array[i]);
		}

		return newArray;
	}

	/**
	 * 将short[] 数组转化成String[]数组
	 *
	 * @param array 
	 * @return String[]
	 */
	public static String[] toStringArray(short[] array) {
		String[] newArray = new String[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = String.valueOf(array[i]);
		}

		return newArray;
	}
	
	/**
	 * 
	 * Description: 将String[] 数组转化成Integer[]数组
	 *
	 * @param array 
	 * @return Integer[]
	 * @throws 
	 * @Author yaodawei
	 * Create Date: 2014-6-6 下午02:36:30
	 */
	public static Integer[] toIntegerArray(String[] array){
		Integer[] newArray = new Integer[array.length];
		for (int i = 0; i < array.length; i++) {
			newArray[i] = Integer.valueOf(array[i]);
		}
		return newArray;
	}

	/**
	 * 将Object[] 数组转化成String[]数组
	 *
	 * @param array 
	 * @return String[]
	 */
	public static String[] toStringArray(Object[] array) {
		String[] newArray = new String[array.length];

		for (int i = 0; i < array.length; i++) {
			newArray[i] = String.valueOf(array[i]);
		}

		return newArray;
	}

}